...
class S3SessionStore(SessionStore):
    ...
    async def get_json_batch(
        ...
        """Fetch metadata JSON for multiple sessions using a single S3 client.

        Args:
            session_ids: List of session identifiers
            key: The JSON key to fetch (e.g., "session.json")
        ...
        async def _fetch_one(client: S3Client, sid: str) -> dict | None:
            try:
                response = await client.get_object(
                    Bucket=self.bucket, Key=self._key(sid, key)
                )
                body = await response["Body"].read()
                return json.loads(body)
            except client.exceptions.NoSuchKey:
        ...
        async with self._client() as client:
            return await asyncio.gather(
                *[_fetch_one(client, sid) for sid in session_ids]
            )
