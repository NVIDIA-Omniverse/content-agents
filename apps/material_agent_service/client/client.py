...
class MaterialAgentClient:
    ...
    def start_pipeline(
        ...
        if session_id:
            ...
            f = open(usd_path, "rb")
            # Keep handle open until request completes
            files.append(
                ("usd_file", (usd_path.split("/")[-1], f, "application/octet-stream"))
            )
        else:
        ...
        if reference_descriptions:
            ...
            data["reference_descriptions"] = json.dumps(list(reference_descriptions))
        ...
        if optimize_usd is not None:
            ...
            if optimize_usd:
                if enable_deinstance is not None:
                    data["enable_deinstance"] = "true" if enable_deinstance else "false"
