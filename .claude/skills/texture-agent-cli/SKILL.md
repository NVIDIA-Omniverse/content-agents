...
description: Run the Texture Agent CLI for AI-driven texture generation on USD materials. Use when user wants to run the texture-agent CLI directly, add textures to a materialized USD asset, generate weathering/aging textures, run the texture pipeline, apply textures per-prim, or create texture variations. Trigger phrases include "texture agent cli", "add textures", "run texture agent", "texture pipeline", "generate textures", "weather the asset", "age the materials", "per-prim textures", "texture variations".
---
...
- Image generation service credentials/endpoints for the configured backend. Public defaults use `GOOGLE_API_KEY` for Gemini or `NVIDIA_API_KEY` for NVIDIA-hosted NIM; local image-gen sidecars require the configured base URL.
- Rendering service endpoint/credentials if `render_previews` or `render` uses a remote render service. For the local OVRTX Docker sidecar, use `RENDER_ENDPOINT=http://localhost:8001`.
...
|--------|------|-------------|
| `apps/texture_agent/configs/texture_example.yaml` | per-material | Public reference config template |
