...
- Environment variables from `.env` must be set:
  ...
  - **Rendering/optimization services**: if the config uses remote rendering or optimization, set `RENDER_ENDPOINT` / `OPTIMIZER_ENDPOINT` to the deployed services plus any auth they require. For the local OVRTX Docker sidecar, use `RENDER_ENDPOINT=http://localhost:8001`.
  - **Generated reference images** (optional): configure the image-gen backend/model and any required API key or endpoint before enabling `generate_reference_image`.
