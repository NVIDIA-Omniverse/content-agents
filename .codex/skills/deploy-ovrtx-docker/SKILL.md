...
By default the compose files publish host port **8001** to container port **8000**. Health check succeeds once the GPU has warmed up (`gpu_initialized: true`).
...
Solution: Stop the other container, or bring OVRTX up via a compose overlay that replaces the published port. OVRTX listens on container port 8000; the host port is the left side of the mapping.

```yaml
# /tmp/ovrtx-port-8010.override.yml
services:
  ovrtx-rendering-api:
    ports: !override
      - "8010:8000"
...
Then apply the overlay with the same base compose file you use to start the service:

```bash
docker compose \
  -f apps/physics_agent_service/docker-compose.yml \
  -f /tmp/ovrtx-port-8010.override.yml \
  up ovrtx-rendering-api --build
```
